...
export default {
    ...
    uploadInUse() {
        ...
        arrayInUse.forEach(function(element, index, array) {
            let cheveretoUploadOnParse = getConfig("cheveretoUploadOnParse")
            if (cheveretoUploadOnParse) {
                _this.uploadImageWithPath(element)
            }
            else {
                atom.notifications.addWarning('Chevereto Upload On Parse was disabled. Please enable it if you want to upload images.', {
                    dismissable: true
                })
            }
